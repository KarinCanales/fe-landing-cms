...
export default function CatalogSection({ sanityData }: Props) {
  ...
  const rowCount = Math.max(1, Math.ceil(galleryEntries.length / 3));
  const baseColumns = '1fr 1fr 1fr';

  const baseRows = useMemo(() => Array.from({ length: rowCount }, () => '1fr').join(' '), [rowCount]);

  const getRowTemplate = useMemo(() => {
    return (activeRow?: number) => {
      if (!activeRow) return baseRows;

      return Array.from({ length: rowCount }, (_, index) => (index + 1 === activeRow ? '1.62fr' : '0.72fr')).join(' ');
    };
  }, [baseRows, rowCount]);

  const getColumnTemplate = useMemo(() => {
    return (activeColumn?: number) => {
      if (!activeColumn) return baseColumns;

      return [1, 2, 3].map((column) => (column === activeColumn ? '1.72fr' : '0.72fr')).join(' ');
    };
  }, [baseColumns]);

  const applyGridTemplate = useMemo(() => {
    return (activeRow?: number, activeColumn?: number) => {
      const gallery = galleryRef.current;
      if (!gallery || !hoverEnabledRef.current) return;

      gallery.style.gridTemplateRows = getRowTemplate(activeRow);
      gallery.style.gridTemplateColumns = getColumnTemplate(activeColumn);
    };
  }, [getColumnTemplate, getRowTemplate]);
  ...
  useEffect(() => {
    ...
    gallery.style.gridTemplateRows = baseRows;
    gallery.style.gridTemplateColumns = baseColumns;
  }, [baseRows, baseColumns]);

  useEffect(() => {
    const mediaQuery = window.matchMedia('(hover: hover) and (pointer: fine) and (min-width: 861px)');

    const handleChange = () => {
      hoverEnabledRef.current = mediaQuery.matches;

      if (!mediaQuery.matches && galleryRef.current) {
        galleryRef.current.style.gridTemplateRows = baseRows;
        galleryRef.current.style.gridTemplateColumns = baseColumns;
      }
    };
        ...
        <div className={styles.container}>
          ...
          <div className={styles.galleryShell}>
            ...
            <motion.div
              ...
              style={{ gridTemplateRows: baseRows, gridTemplateColumns: baseColumns }}
              onPointerLeave={() => applyGridTemplate()}
            >
              {galleryEntries.map((entry, galleryIndex) => {
                const media = entry.media;
                const rowIndex = Math.floor(galleryIndex / 3) + 1;
                const columnIndex = (galleryIndex % 3) + 1;
                ...
                return (
                  <motion.article
                    ...
                    tabIndex={0}
                    onPointerEnter={() => applyGridTemplate(rowIndex, columnIndex)}
                    onFocus={() => applyGridTemplate(rowIndex, columnIndex)}
                    onBlur={() => applyGridTemplate()}
