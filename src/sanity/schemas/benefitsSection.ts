import {cardSizes, iconList, overlayPresets} from './shared';
...
const benefitsSection = {
  ...
  fields: [
    ...
    {name: 'backgroundAlt', title: 'Descripción de la imagen para accesibilidad', type: 'string', group: 'background', description: 'Texto que describe la imagen de fondo.', options: fieldHelp({example: 'Decoración elegante con flores y mesa de evento.'})},
    {name: 'overlay', title: 'Filtro visual del fondo', type: 'string', group: 'background', description: 'Capa visual que ayuda a armonizar la imagen con la paleta.', options: {list: overlayPresets.map((p) => p), ...fieldHelp({example: 'Claro editorial almendra o Luz cálida suave.'})}},
    ...
    {name: 'description', title: 'Texto descriptivo', type: 'text', group: 'content', description: 'Párrafo de apoyo que explica la promesa de la sección.', options: fieldHelp({helpText: 'Idealmente 1 a 3 líneas para mantener buen ritmo visual.'})},
    {
      ...
      of: [{
        ...
        fields: [
          ...
          {name: 'icon', title: 'Icono', type: 'string', description: 'Icono decorativo del beneficio.', options: {list: iconList.map((i) => i), ...fieldHelp({helpText: 'Elige un icono de la lista.'})}},
        ...
        preview: {select: {title: 'title', subtitle: 'eyebrow', visible: 'visible'}, prepare: ({title, subtitle, visible}: any) => ({title: title || 'Beneficio', subtitle: visibilitySubtitle(visible, subtitle)})},
      }],
    },
    {
      name: 'stats',
