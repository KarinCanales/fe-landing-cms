...
const testimonialsSection = {
  ...
  fields: [
    ...
    {name: 'backgroundAlt', title: 'Descripción de la imagen para accesibilidad', type: 'string', group: 'background', description: 'Texto que describe la imagen de fondo.', options: fieldHelp({example: 'Fondo cálido con velas para testimonios.'})},
    {name: 'overlay', title: 'Filtro visual del fondo', type: 'string', group: 'background', description: 'Capa visual que armoniza la imagen con la sección.', options: {list: overlayPresets.map((p) => p), ...fieldHelp({example: 'Luz cálida suave.'})}},
